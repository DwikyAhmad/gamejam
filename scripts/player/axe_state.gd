...
func _on_next_transitions() -> void:
    ...
    if !animated_sprite_2d.is_playing():
        ...
        var axe_position = player.global_position

        # Offset berdasarkan arah player
        if !animated_sprite_2d.flip_h:
            axe_position.x += 16 # Gali ke kanan
        else:
            axe_position.x -= 16 # Gali ke kiri
            
        # Konversi posisi global ke posisi lokal tilemap
        var local_pos = ground_tilemap.to_local(axe_position)
        # Konversi posisi lokal ke koordinat tile
        var tile_pos = ground_tilemap.local_to_map(local_pos)
        
        print("Global pos: ", axe_position)
