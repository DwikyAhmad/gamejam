...
var harvest_goal = {"current": 0, "target": 3}
...
func increment_harvest():
    harvest_goal.current = min(harvest_goal.current + 1, harvest_goal.target)
    emit_signal("goal_updated", "harvest", harvest_goal.current, harvest_goal.target)
    _check_all_goals_completed()
...
func _check_all_goals_completed():
    if not goals_completed and \
       ...
       seed_goal.current >= seed_goal.target and \
       harvest_goal.current >= harvest_goal.target:
